...
namespace Tests
{
    ...
    public class SingleBeatsTest
    {
        ...
        public async Task TestNoBeat()
        {
            ...
            List<IMoveable> availableMoves = await scope.FindMoves(p1);
            List<IMakeBeat> fightMoves = availableMoves.Where(x => x is IMakeBeat).ToList().ConvertAll(obj => (IMakeBeat)obj);
            Assert.IsTrue(fightMoves.Count == 0);
        }
        ...
        public async Task TestOneBeat()
        {
            ...
            List<IMoveable> availableMoves = await scope.FindMoves(p1);
            List<IMakeBeat> fightMoves = availableMoves.Where(x => x is IMakeBeat).ToList().ConvertAll(obj => (IMakeBeat)obj);
            Assert.IsTrue(fightMoves.Count == 1);
        }
        ...
        public async Task TestTwoBeat() //2 enemies in different way
        {
            ...
            List<IMoveable> availableMoves = await scope.FindMoves(p1);
            List<IMakeBeat> fightMoves = availableMoves.Where(x => x is IMakeBeat).ToList().ConvertAll(obj => (IMakeBeat)obj);
            Assert.IsTrue(fightMoves.Count == 2);
        }
        ...
        public async Task EnemyArround() //enemies in ea direction
        {
            ...
            List<IMoveable> availableMoves = await scope.FindMoves(p1);
            List<IMakeBeat> fightMoves = availableMoves.Where(x => x is IMakeBeat).ToList().ConvertAll(obj => (IMakeBeat)obj);
            Assert.IsTrue(fightMoves.Count == 4);
        }
        ...
        public async Task TestTwoBeatKing()
        {
            ...
            List<IMoveable> availableMoves = await scope.FindMoves(p1);
            List<IMakeBeat> fightMoves = availableMoves.Where(x => x is IMakeBeat).ToList().ConvertAll(obj => (IMakeBeat)obj);
            Assert.IsTrue(fightMoves.Count == 3);
        }
